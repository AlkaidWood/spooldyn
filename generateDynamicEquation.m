...
"EDiskPhase = Parameter.Matrix.eccentricityPhase;"
];
else
loadMatrix1 = [...
" ";...
...
['Q([', num2str(diskDof), '])   = EDisk .* (domega(diskInShaftNo).^2 .* cos(omega(diskInShaftNo)+EDiskPhase));'];...
['Q([', num2str(diskDof+1), ']) = EDisk .* (domega(diskInShaftNo).^2 .* sin(omega(diskInShaftNo)+EDiskPhase));'];...
 ' '...
};
else
processQ = {...
 ...
 '% calculate unblance force';...
['diskInShaftNo = [', num2str(diskInShaftNo), '];'];...
['Q([', num2str(diskDof), '])   = EDisk .* ( ddomega(diskInShaftNo) .* sin(omega(diskInShaftNo)+EDiskPhase) + domega(diskInShaftNo).^2 .* cos(omega(diskInShaftNo)+EDiskPhase));'];...
['Q([', num2str(diskDof+1), ']) = EDisk .* (-ddomega(diskInShaftNo) .* cos(omega(diskInShaftNo)+EDiskPhase) + domega(diskInShaftNo).^2 .* sin(omega(diskInShaftNo)+EDiskPhase));'];...
 ' '...
};
end
