...
Shaft.amount            = 1;
Shaft.totalLength       = [517.2]*10^-3; % all vectors in column (m)
...
Shaft.rayleighDamping   = [0, 1.9e-4]; % [alpha, beta] CShaft = alpha*(MShaft+MDisk) + beta*KShaft

%%

% typing the parameter about running status
Status.ratio            = []; % [v-shaft2/v-shaft1; v-shaft3/v-shaft1]
Status.vmax             = 3000 / 60 * (2*pi); % rad/s, the maximum rotational speed for shaft 1
Status.acceleration     = 60 / 60 * (2*pi); % rad/s^2, acceleration of shaft 1
...
Disk.outerRadius        = [37.635, 37.635]' * 10^-3; % m 
Disk.thickness          = [23.6, 23.6]' * 10^-3; % m
Disk.positionOnShaftDistance = [208.6, 308.6]' *10^-3; %from left end (m)
Disk.density            = 7850*ones(Disk.amount, 1); % kg/m^3
Disk.eccentricity       = [0.1012, 0.8287]' * 1e-4; % m
Disk.eccentricityPhase  = [0, 0]'; % rad/s
...
Bearing.stiffness       = [9e7,  9e7]'; % N*m
Bearing.stiffnessVertical = [9e7,  9e7]'; % N*m
Bearing.damping         = [3.52e4, 7.5619e6]'; % N*s/m
Bearing.dampingVertical = [3.52e4, 7.5619e6]'; % N*s/m
Bearing.mass            = [0, 0]'; % kg
