%% inputEssentialParameterSingleEnd - Input essential parameters for a single-rotor system
%
% This function initializes and returns the essential parameters for 
% modeling a single-rotor system, including shaft configurations, disk 
...
%
%% Syntax
%  InitialParameter = inputEssentialParameterSingleEnd()
%
%% Description
% |inputEssentialParameterSingleEnd()| initializes parameters for shafts, disks, 
...
% single-rotor system in subsequent dynamic analysis. All vectors should 
% be in column format.
%
%% Output Parameters
...
%
%% Example
%   InitialParams = inputEssentialParameterSingleEnd();
...
%%
function InitialParameter = inputEssentialParameterSingleEnd()
...
Disk.amount             = 1; % [propeller, motor]
Disk.inShaftNo          = [1]'; % disks in the i-th shaft
Disk.dofOfEachNodes     = 4 * ones(Disk.amount,1);
Disk.innerRadius        = [5]' * 10^-3; % m
Disk.outerRadius        = [50]' * 10^-3; % m 
Disk.thickness          = [25]' * 10^-3; % m
Disk.positionOnShaftDistance = [500]' *10^-3; %from left end (m)
Disk.density            = 7850*ones(Disk.amount, 1); % kg/m^3
Disk.eccentricity       = [0.2]' * 1e-3; % m
...
Bearing.positionOnShaftDistance = [0, 400]' * 10^-3; % m
